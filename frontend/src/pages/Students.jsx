...
import api, { resolveBackendFileUrl } from '../api/axios';
import {
...
import { getApiErrorMessage, isStrongPassword, isValidUzPhone, normalizeUzPhone } from '../utils/formUtils';
...
export default function Students() {
  ...
  async function saveStudent() {
    try {
      if (!form.full_name || !form.email || !form.password || !form.phone || !form.address || !form.birth_date) {
        toast.error("Barcha majburiy maydonlarni to'ldiring!");
        return;
      }
      if (!isValidUzPhone(form.phone)) {
        toast.error("Telefon raqam formati noto'g'ri. Masalan: +998901234567");
        return;
      }
      if (!isStrongPassword(form.password)) {
        toast.error("Parol kamida 8 ta belgi, katta-kichik harf, raqam va belgi bo'lishi kerak.");
        return;
      }
      ...
      formData.append('phone', normalizeUzPhone(form.phone));
      ...
      toast.error(getApiErrorMessage(err, "Talabani saqlab bo'lmadi."));
    }
  }
  ...
  async function updateStudent() {
    try {
      ...
      toast.error(getApiErrorMessage(err, "Talabani yangilab bo'lmadi."));
    }
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(err, "Talabani arxivga yuborib bo'lmadi."));
    }
  };
  ...
  async function restoreStudent(id) {
    try {
      ...
      toast.error(getApiErrorMessage(err, "Talabani arxivdan chiqarib bo'lmadi."));
    }
  }
                  ...
                  <TableRow key={student.id} hover sx={{ '&:last-child td': { border: 0 } }}>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Avatar src={student.photo ? resolveBackendFileUrl(student.photo) : undefined} sx={{ width: 32, height: 32, backgroundColor: getColor(student.id), fontSize: '0.75rem', fontWeight: 700 }}>{getInitials(student.full_name)}</Avatar>
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <KeyIcon sx={{ fontSize: 18, color: '#9ca3af' }} />
                    </InputAdornment>
                  ),
                  endAdornment: (
