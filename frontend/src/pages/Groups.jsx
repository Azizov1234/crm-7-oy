...
export default function Groups() {
  ...
  async function fetchGroups() {
    try {
      const statusQuery = activeTab === 'archive' ? '?status=cancelled' : '';
      const res = await api.get(`/api/v1/groups${statusQuery}`);
  ...
  async function saveGroup() {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(e, "Guruhni saqlab bo'lmadi."));
  ...
  async function updateGroup() {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(e, "Guruhni yangilab bo'lmadi."));
    } finally { setSaving(false); }
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(e, "Guruhni arxivga yuborib bo'lmadi."));
    }
  };
  ...
  async function restoreGroup(id) {
    try {
      ...
      toast.error(getApiErrorMessage(e, "Guruhni arxivdan qaytarib bo'lmadi."));
    }
  }

  async function handleGroupStatusToggle(group, checked) {
    const nextStatus = checked ? "active" : "cancelled";
    if (group.status === nextStatus) return;

    try {
      setStatusUpdatingId(group.id);
      await api.put(`/api/v1/groups/${group.id}`, { status: nextStatus });
      await fetchGroups();
      toast.success(checked ? "Guruh active holatga o'tdi." : "Guruh inactive (arxiv) holatga o'tdi.");
    } catch (e) {
      toast.error(getApiErrorMessage(e, "Guruh statusini o'zgartirib bo'lmadi."));
    } finally {
                  ...
                  <TableRow key={group.id} hover sx={{ '&:last-child td': { border: 0 } }}>
                    {/* Status */}
	                    <TableCell>
                          ...
                          {(() => {
                            const isEnabled = group.status === 'active' || group.status === 'planned';
                            return (
                              <>
	                        <Switch
	                          size="small"
	                          checked={isEnabled}
                              disabled={statusUpdatingId === group.id}
                              onChange={(e) => handleGroupStatusToggle(group, e.target.checked)}
	                          sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#3b82f6' }, '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { backgroundColor: '#93c5fd' } }}
	                        />
	                        <Typography sx={{ fontSize: '0.68rem', fontWeight: 700, color: isEnabled ? '#2563eb' : '#ef4444' }}>
