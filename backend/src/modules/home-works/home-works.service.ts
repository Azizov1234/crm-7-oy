...
export class HomeWorksService {
  ...
  async update(
    ...
    if (file) {
      if (hw.file) {
        const oldPath = join(process.cwd(), "uploads", hw.file);
        try {
    ...
    if (video) {
      if (hw.video_url) {
        if (hw.video_url.includes("supabase.co/storage")) {
          ...
          const oldPath = join(process.cwd(), "uploads", hw.video_url);
          try {
            fs.unlinkSync(oldPath);
          } catch {}
        }
  ...
  async remove(id: number, currentUser: { id: number; role: UserRole }) {
    ...
    if (hw.file) {
      const filePath = join(process.cwd(), "uploads", hw.file);
      try {
        fs.unlinkSync(filePath);
      } catch {}
      if (url && key) {
    ...
    if (hw.video_url && hw.video_url.includes("supabase.co/storage")) {
      ...
      const filePath = join(process.cwd(), "uploads", hw.video_url);
      try {
        fs.unlinkSync(filePath);
      } catch {}
    }
